...
import { Lock, User as UserIcon, Shield, Check, X, LogIn, Key, RefreshCw, Eye, EyeOff, Settings } from 'lucide-react';
  ...
  const handleCustomLogin = (e: React.FormEvent) => {
    ...
    if (found) {
      if (found.status === 'Suspended') {
        setError(`Account "${found.username}" is suspended. Contact System Administrator.`);
        return;
      }
      const requiredPass = found.password || `${found.username}123`;
      if (password && password.trim() !== requiredPass) {
        setError('Incorrect password. Please verify your credentials or click a role profile below.');
        return;
      }
      onLogin(found);
      onClose();
    } else {
      setError('User account not found. You can also click any pre-configured role below.');
    }
  };
              ...
              {users.map((user) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isActive ? (
                      <Check className="w-4 h-4 text-[#123b5d] shrink-0" />
                    ) : (
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full pl-9 pr-10 py-2 text-xs border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#123b5d]"
                />
                <button
                  type="button"
            ...
            {onManageUsers && (
              <div className="pt-2 text-center border-t border-slate-100">
                <button
                  ...
                  className="text-[11px] text-[#123b5d] hover:underline font-semibold flex items-center justify-center gap-1 mx-auto"
                >
                  <Settings className="w-3 h-3" />
                  <span>Configure User Credentials & Permissions</span>
                </button>
              </div>
            )}
          </form>
